...
function openComment() {
    $.ajax({
         url:ROOT_URL+"/admin/event/getComment",
         type: "post",
         data:{visiter_id:visiter_id,business_id},
         success:(res)=>{
        
           if(res.code==0){
                  let html = commentStr(res.data);
        ...
        $("#picture").ajaxSubmit({
            ...
            success: function (res) {
                if(res.code == 0){
                    ...
                    var str =  getOuterRight({content:msg,pic:pic,time:formattingTime(myDate),})
...
function mp4Put() {
    ...
    $("#mp4Form").ajaxSubmit({
        ...
        url: ROOT_URL + '/admin/event/uploadMp4',
                ...
                if(res.code == 0){
                    ...
                    res.data.forEach(v=>{
                        if(res.data.indexOf('.mp4')>= 0){
                            str += "<video src='" + v.url + "' controls='controls' style='width: 100%'>ERROR</video>";
                        }else{
                            str += "<div><a href='" + v.url + "' style='display: inline-block;text-align: center;min-width: 70px;text-decoration: none;' download='" + name + "'><i class='layui-icon' style='font-size: 60px;'>&#xe61e;</i><br>" + name + "</a></div>";
                        }
                    })

                   str =  getOuterRight({content:str,time:formattingTime(myDate),pic:pic})
                    $(".conversation").append(str);
                    var div = document.getElementById("wrap");
                    div.scrollTop = div.scrollHeight;
                    setTimeout(function(){
            ...
            if(res.code == 0){
                ...
                $.each(res.data, function (k, v) {
                    ...
                    if (v.direction == 'to_service') {
                        v.pic = v.avatar;
                        v.time = formattingTime(new Date(v.timestamp*1000));
                       str +=  getOuterRight(v);
...
function gettishi(business_id){
    ...
    $.ajax({
        ...
        success:function(res){
            if(res.code == 0 && is_init){
                is_init = false;

              var d = new Date();
                  let time = formattingTime(d);
                var num = 0;
...
function zhuanrengong(business_id){
    $.ajax({
        ...
        success:function(res){
            if(res.code == 0){
                 var str='';
                str += "<p style='font-weight:blod;'>你好！已成功为您连线人工，请选择您的客服专员:</p><ul>";
                var num ='';
    ...
    $.ajax({
        ...
        success: function (res) {
            if(res.code == 0){
                ...
                // 问候语
                data.time = formattingTime(new Date());
                for (const content of data.content) {
                    data.content = content;
                    var msg = getOuterLeft(data)
...
function getanswer(id,question){
    var str = getOuterRight({content:question,time:formattingTime(new Date()),pic:pic})
    $(".conversation").append(str);
    $.ajax({
...
var send = function () {
    ...
    if (msg == '' || $.cookie("service") == '') {
        ...
        var str =  getOuterRight({content:msg2,time:formattingTime(myDate),pic:pic,is_read:0,cid:'00' })
