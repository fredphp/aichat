    ...
    $("#mp4Form").ajaxSubmit({
        ...
        success: function (res) {
            if (res.code == 0) {
                ...
                var str = '';
                var msg = `<div class="mp4-body">
  <video  class='chat-mp4 video-js' crossorigin='anonymous'src="${res.data}" preload='metadata' controls data-setup='{}' playsinline x5-playsinline webkit-playsinline  ></video>
</div>`;
                ...
                str += "<button onclick=revoke('" + unstr + "',2); class='revoke-text'>撤销</button>";

                var str = getOuterRight({cid:unstr, pic: imghead, time: time, content: msg+str});
                        ...
                        recorder && recorder.exportWAV(function(blob) {
                            ...
                            xhr.onreadystatechange = function() {
                                ...
                                if (xhr.readyState == 4 && xhr.status == 200) {
                                    ...
                                    var str =  getOuterRight({pic: pic, time: formattingTime(myDate), content: voicemessage});
        ...
        success:function (res) {
            if(res.code == 0){
                layer.msg(res.msg,{icon:1,end:function(){
                        // $("#xiaox_"+id).remove();
                        // 使用 ID 删除消息
                        var msgElement = $("#xiaox_" + id);  // 确保 ID 正确拼接
                        if (msgElement.length) {
                            msgElement.remove();  // 删除该元素
                        } else {
                            console.log("未找到对应的消息元素");}
                    }});
            }else{
                layer.msg(res.msg,{icon:2});
            }
                ...
                $.each(res.data, function(k, v) {
                    ...
                    if (v.direction == 'to_service') {
                        if(v.content.indexOf('<img')>= 0||v.content.indexOf('<a')>= 0||!isNaN(v.content)||v.content.indexOf('<video')>= 0){
                            ...
                            if(v.content_trans !='' && translators_state=='open'){
                                content += v.content+"<p class='trans-data'>译文："+v.content_trans+"</p>";
                            }else if(translators_state=='open'){
                                content +=  v.content+"<span class='trans' data-cid='"+v.cid+"'>翻 译</span>";
                            }
                            else{
                            content = v.content;
                        }
                        }
                       str +=  getOuterLeft({content:content,type:1,time:formattingTime(myDate),nick_name:'',avatar:v.avatar})

                    } else {
                        content =  v.content + "<button onclick=revoke("+v.cid+",1); class='revoke-text'>撤销</button>";
                        str +=  getOuterRight({cid:v.cid,time:formattingTime(myDate),content:content,pic:v.avatar});
        ...
        $("#picture").ajaxSubmit({
            ...
            success: function(res) {
                if (res.code == 0) {
                    ...
                    str += "<button onclick=revoke('" + unstr + "',2); class='revoke-text'>撤销</button>";
                    str = getOuterRight({cid:unstr,pic:imghead,content:msg+str,time:formattingTime(myDate)});
        ...
        $("#file").ajaxSubmit({
            ...
            success: function(res) {
                if (res.code == 0) {
                    ...
                    str += "<button onclick=revoke('" + unstr + "',2); class='revoke-text'>撤销</button>";
                    str = getOuterRight({cid:unstr,pic:imghead,content:str,time:formattingTime(myDate)});
...
var send = function() {
    ...
    if (msg == "") {
        ...
        var unstr=(new Date()).valueOf()+randomChar(5)+visiter_id;
        var str = '';
        str +=  msg + "&nbsp;&nbsp;<button onclick=revoke('" + unstr + "',2); class='revoke-text'>撤销</button>";
        str = getOuterRight({cid:unstr,pic:imghead,content:str,time:formattingTime(new Date())})
...
$(document).on('click','.outer-left .customer img,.outer-right .service img,.outer-right .customer img,.outer-left .service img',function(e) {
    var that = this;
    var img = new Image();
    img.src = this.src
    // 如果图片被缓存，则直接返回缓存数据
