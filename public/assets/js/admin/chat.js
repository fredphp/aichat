...
function listenClientMsg(){
    let subscribe = pusher.subscribe("read_client_msg" + value);
    subscribe.bind("on_notice", function (data) {
...
time = formattingTime(new Date())
        ...
        var str = msg + "&nbsp;&nbsp;<button onclick=revoke('" + unstr + "',2); class='revoke-text'>撤销</button";
        str = getOuterRight({pic:pic,cid:unstr,content:str,time:time});
...
var send = function () {
 ...
 /*  var reg = new RegExp( '<' , "g" )``
    ...
    if (msg == "") {
        ...
        var unstr=(new Date()).valueOf()+randomChar(5)+sid;
        var str = '';
        str +=  msg + "<button onclick=revoke('" + unstr + "',2); class='revoke-text'>撤销</button></pre>";
        str = getOuterRight({pic:pic,time:formattingTime(new Date()),cid:unstr,content:str})
        $(".conversation").append(str);
        $("#text_in").val('');
        var div = document.getElementById("wrap");
        div.scrollTop = $('.conversation')[0].scrollHeight;
        ...
        $("#file").ajaxSubmit({
            ...
            success: function (res) {
                if(res.code == 0){
                    ...
                    str = getOuterRight({pic:pic,time:formattingTime(new Date()),content:str})
                    $(".conversation").append(str);
                    var div = document.getElementById("wrap");
                    div.scrollTop = $('.conversation')[0].scrollHeight;
                    $(".chatmsg").css({
        ...
        $("#picture").ajaxSubmit({
            ...
            success: function (res) {
                if(res.code == 0){
                    ...
                    time = formattingTime(new Date())
                    var str = getOuterRight({pic:pic,time:time,content:msg});
...
$(document).on('click','.outer-left .service img,.outer-right .service img, .outer-right .customer img',function(e) {
    var that = this;
    var img = new Image();
    img.src = this.src
    // 如果图片被缓存，则直接返回缓存数据
