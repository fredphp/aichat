        ...
        success: function (res) {
            if (res.code == 0) {
                ...
                $.each(data, function (k, v) {
                    ...
                    a += ' <span style="font-size: 14px;color: #555555;line-height: 80px;margin-left: 82px">' + getNickName(v) + '</span><div style="position:absolute;right:0;top:30px;cursor: pointer;" onclick="recovery(' + "'" + v.visiter_id + "'" + ')"><img src="'+'/assets/images/admin/B/delete.png"></img></div></div>';
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.code == 0) {
                var data = res.data;
                var str = data[0]; // 国家
                if (data[1]) str += " 、" + data[1]; // 省
                if (data[2]) str += " 、" + data[2]; // 市
                $(".iparea").text(str);
            }
        }
    })
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.code == 0) {
                ...
                user.visiter_name=user.location+'-'+user.ip;
                ...
                str +='<a style="background-color:#bf1c06" class="layui-btn layui-btn-sm   chat2top js-ajax-btn"  onclick="clearList()" target=_blank "><i class="layui-icon  layui-icon-delete"></i>清空列表</a>';
                str +='<a style="background-color:#0aa073" class="layui-btn layui-btn-sm  chatexport chat2top js-ajax-btn"  href="/admin/index/export?visiter_id='+user.visiter_id+'" target=_blank "><i class="layui-icon layui-icon-export"></i>导出消息</a>';
                 ...
                 str +=`<button style="background-color:#c10cbf" class="layui-btn layui-btn-sm  chatexport chat2top js-ajax-btn" id="searchMessage" data-vid='${user.visiter_id}'   menu-title="消息搜索 "><i class="layui-icon layui-icon-search"></i>消息搜索</button>`;
                ...
                $.each(data, function (k, v) {
                    ...
                    getdata.puttime = v.timestamp;
                     myDate = new Date(v.timestamp*1000);
