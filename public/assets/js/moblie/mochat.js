    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.code == 0 && res.data.length > 0) {
                ...
                let question_key_list = $("#question_key_list");
                let str = '<p style="font-weight:bold;">' + guess_ask + '</p>';  // “我猜你想问的”
                ...
                $.each(res.data, function (k, v) {
                    ...
                    if (v.keyword !== '') {
                        question_key_list.append(
                            '<a class="slideInRightItem" onclick="getanswer(' + v.qid + ',`' + v.question + '`)">' + v.keyword + '</a>'
                        );
                    }
                });
                ...
                // 没有推荐问题，隐藏关键词区域
                $('.keyword-toggle').hide();
                $("#question_key_list").remove(); // 移除列表容器（可选）
            }
        }
    });
...
function tz(business_id,groupid,id){
    ...
    // 跳转到 kf5.xywlgzs.vip/application/mobile/view/index
