<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:79:"/www/wwwroot/kf5.xywlgzs.vip/public/../application/mobile/view/admin/index.html";i:1741801771;}*/ ?>
<!DOCTYPE html>
<html lang="zh-cn">
<head>
    <meta charset="UTF-8" />
   ...
   .wechat__tabBar_dibu ul li.swiper-pagination-bullet-active span {
            color: #0583f8;
        }
        .wechat__tabBar {
            position: fixed;
        ...
        .wc__ucenter-list ul li .person-info {
            ...
            top: 0;
            left: 0;
            background: linear-gradient(-45deg, #0f0c29, #302b63, #24243e, #191970);
            background-size: 400% 400%;
            animation: darkBgFlow 6s infinite alternate ease-in-out;
            box-shadow: 0px 0px 15px 0px rgba(0.0.0.3);
        }
          @keyframes darkBgFlow {
...
<h2 class="barTit barTitLg flex1 auto-play"><img class="logo" src="<?php echo $mine['avatar']; ?>" width="30px" height="30px" top="30px" style="border-radius:5px" line-height="30px">&nbsp;&nbsp;hi&nbsp;&nbsp;</a>(<em style="color: #fff;"><?php echo $mine['user_name']; ?>)</em>

               <a href="/service" target="_blank"><span class="layui-icon" style="float:right">&nbsp;&#xe656;&nbsp;&nbsp;<i class="el-icon-position"></i></span></a><a href="/mobile/admin" target="_blank"><span class="layui-icon" style="float:right">&nbsp;&#xe63b;&nbsp;&nbsp;</span></a></h2>
            </div>
        </div>
        <!-- //底部tabbar -->
        ...
        <!-- //4个tabBar滑动切换 -->
        <div class="wc__swiper-tabBar flex1" style="margin-top: 0">
                    ...
                    <div class="swiper-slide"  data-history="my">
                        <div class="wc__scrolling-panel">
                            <div class="wc__ucenter-list" id="J__ucenterList">
                                <ul class="clearfix">
                                    <li>
                                        <div class="item flexbox flex-alignc wc__material-cell person-info" style="">
                                            <img class="uimg" src="<?php echo $mine['avatar']; ?>" />
                                            <span class="txt username flex1">
                                                    <em style="color: #fff"><?php echo $mine['nick_name']; ?></em><i style="color: #fff">商户名：<?php echo $mine['user_name']; ?></i>
                                            </span>
                                        </div>
                                    </li>
                                    ...
                                    <li class="auto-play">
                                        <div class="item flexbox flex-alignc wc__material-cell">
                                            ...
                                            <span class="txt flex1">语音提示</span>
                                 ...
                                 <li style="border-top: 1px solid #ebeff0" routeUrl="<?php echo url('mobile/certification/index'); ?>">
                                        <div class="item flexbox flex-alignc wc__material-cell">
                                            <img style="height: 0.363rem;width: 0.363rem;margin-right: 0.217rem" src="/index/images/rz.png" alt="">
                                            <span class="txt flex1">实名认证</span>
                                      
                                        </div>
                                    </li>
                                          <li style="border-top: 1px solid #ebeff0" >
                                        <div class="item flexbox flex-alignc wc__material-cell"  >
                                            <img style="height: 0.363rem;width: 0.363rem;margin-right: 0.217rem" src="/index/images/qk.png" alt=""layui-icon layui-icon-layer"">
                                            <span  onclick="clearList();" class="txt flex1">清空消息列表</span>
        ...
        <!-- //底部tabbar -->
<!--<footer>-->
<!--    <div class="layui-icon layui-icon-reply-fill"  style="font-size:0.5rem;  margin-top:-5px;color:#0583f8;" onClick="tz('/mobile/admin')">-->
<!--        <p style="font-size:14px; margin: -5px; color: #0583f8;">访客</p></div>-->
<!--        <div class="layui-icon layui-icon-user" style="font-size:0.45rem;  margin-top:-5px; color: #999;" onClick="tz('/mobile/admin/user?id=0')"><p style="font-size:14px; margin: -5px; color: #999;">用户</p></div>-->
...
<script type="text/javascript">
    var chatSwiper = new Swiper('.swiper-container',{
        pagination: '.wechat-pagination_dibu',
      ...
      paginationBulletRender: function (chatSwiper, index, className) {
            switch (index) {
                case <?php echo $i=0; ?>:
                    name='<span class="wechat-text">对话</span>';
                    break;
     

            case <?php echo ++$i; ?>:
                name='<span class="wechat-text">我的</span>';
                break;
            default: name='';
            }
...
<style>
    .flex1.swiper-pagination-bullet{
       display: flex;
       align-items: center;
        position: relative;
    }
    ...
    var chatSwiper = new Swiper('.swiper-container',{
        ...
        paginationBulletRender: function (chatSwiper, index, className) {
            switch (index) {
                case <?php echo $i=0; ?>:
                    name=`<span class="wechat-text">对话</span>
                    <?php if(isset($kefu_select_state) && $kefu_select_state == 'open'): ?>
                          <select id="selectKefu" style="width: 20px;position: absolute;right: 0;top: 50%;transform: translateY(-50%);-webkit-transform: translateY(-50%);-moz-transform: translateY(-50%);-ms-transform: translateY(-50%);-o-transform: translateY(-50%);border: unset;background:inherit">
                              <option selected="selected" disabled="disabled"  style='display: none' value=''></option>
                                <?php foreach($allService as $v): ?>
                                    <option  value="<?php echo $v['service_id']; ?>"><?php echo $v['nick_name']; ?></option>
                                <?php endforeach; ?>
...
`;
                    break;
                case <?php echo ++$i; ?>:
            name='<span class="wechat-text">等待</span>';
            break;
        case <?php echo ++$i; ?>:
            name='<span class="wechat-text">分组</span>';
            break;
            case <?php echo ++$i; ?>:
                name='<span class="wechat-text">我的</span>';
                break;
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        layer.open({
            ...
            area: ['', '180px'],
            title:'',
            content: '<div style="text-align:center;margin: 50px 0 30px;font-size:14px;">提示：清空当前所有对话列表，消息内容不会删除额~</div>',
            btn: ['确定', '取消'],
            yes:function(res){
                $.ajax({
                    url:"/admin/set/clear",
                    ...
                    success: function (res) {
                            ...
                            layer.closeAll();
                           window.location.href = ROOT_URL + '/mobile/admin/'
        ...
        function count_unaccept() {
            ...
            const channelOnLoad = (service_id)=>{
                var channel = pusher.subscribe("kefu" + service_id);
                // 发送一个推送
            ...
            const channelUnOnLoad = (service_id)=>{
                pusher.unsubscribe("kefu" + service_id);
            }
            const channelmeOnLoad = (service_id)=>{
                var channelme = pusher.subscribe("ud" + service_id);
            ...
            const channelmeUnOnLoad = (service_id) => {
                pusher.unsubscribe("ud" + service_id);
            }

            const channelallOnLoad = (businessId)=>{
                var channelall = pusher.subscribe("all" + businessId);
            ...
            const channelallUnOnLoad = (businessId)=>{
                pusher.unsubscribe("all" + businessId);
            }
            pusher.connection.bind('state_change', function (states) {
                ...
                if (states.current == 'unavailable' || states.current == "disconnected" || states.current == "failed") {
                    ...
                    init(config.service_id,config.business_id);
                }
        ...
        const init = (service_id,business_id)=>{
            channelmeOnLoad(service_id)
            channelallOnLoad(business_id)
            channelOnLoad(service_id)
        }
        const closeSubscribe = (service_id,business_id)=>{
            channelmeUnOnLoad(service_id)
            channelallUnOnLoad(business_id)
            channelUnOnLoad(service_id)
        }
        init(config.service_id,config.business_id);
        const reloadConnect = (serviceId)=>{
            closeSubscribe(config.service_id,config.business_id)
            switchKeFu(serviceId)
            config.service_id = serviceId
            init(serviceId,config.business_id)
        }
        function switchKeFu(service_id) {
            $.ajax({
                url:"/service/chat/switchKeFu",
                type:"post",
                data:{service_id},
                dataType:'json',
                success:function (res) {
    ...
    $(document).on("change", "#selectKefu", function(){
         reloadConnect($("#selectKefu").val())
    })
