...
//        $arr = $this->request->post('',null,null);
        ...
        if(strpos( $arr['content'],"audio") !== false){
                 $arr['content'] = str_replace('<div style="text-align:center;">','<div style="cursor:pointer;text-align:center;" onclick="getstate(this)" data="play">', $arr['content']);
    ...
    public function isTrans($text,$to,$business_id){
        ...
        // if(strpos($text, '<aud') !== false) return $text;
...
//wolive_chats 消息表
    ...
    public function chatdata()
    {
        ...
        if (!empty($login['last_visiter_id']) && $login['last_visiter_id']!=$post['visiter_id']){
            $lastChannel = bin2hex($login['last_visiter_id'] . '/' . $login['business_id']);
            $pusher->trigger('cu' . $lastChannel, 'close_read_client', ['message' => '']);
        }
        $channel = bin2hex($post['visiter_id'] . '/' . $login['business_id']);
        $pusher->trigger("cu".$channel,'read_client',['message'=>'']);
        $data = ['code' => 0, 'data' => $result,'user'=>$user];
        $_SESSION['Msg']['last_visiter_id'] = $post['visiter_id'];
        return $data;
    }
    ...
    public function getipinfo()
    {
        ...
        $ip = $post['ip'];
        $data = Ip::find($ip);
        if (is_array($data) && isset($data[1], $data[2]) && $data[1] === $data[2]) {
            $data[2] = '';
        }
                ...
                visiter_id:(typeof ai_service=='undefined' || typeof ai_service.visiter_id == 'undefined')?'':ai_service.visiter_id,
                     ...
                     open:function(){
                        ...
                        if(!d){
                            ...
                            document.getElementById('minblzxmsgtitlecontainer_yuan').style.backgroundColor='{$xfbg}';
                                    ...
                                    document.getElementById('blzxMinChatWindowDiv_yuan').style.right='15px';
                                    document.getElementById('blzxMinChatWindowDiv_yuan').style.bottom='20px';
                                    // document.getElementById('blzxMinChatWindowDiv_yuan').style.margin='15px';
                                    
                            const img = document.getElementById('minblzxWinlogo_yuan');
                            ...
                            div.addEventListener('mousedown', (e) => {
                                ...
                                div.style.cursor = \"blzxMinChatWindowDiv_yuan\";
                            });
                     ...
                     document.getElementById('blzxMinChatWindowDiv_yuan').style.display=\"none\";
                      ...
                      var moblieweb = encodeURI('{$url}{$this->base_root}/mobile/index/home?{$theme}visiter_id='+blzx.visiter_id+'&visiter_name='+blzx.visiter_name+'&avatar='+blzx.avatar+'&business_id={$business_id}&groupid='+groupid+'&product='+blzx.product + '&special=' + {$service_id});
                       ...
                       var z =document.getElementById('css1');
                        
                       if(!s){

                       var css1 = document.createElement('div');
                       css1.id =\"css1\";
                       css1.style.width = '100%';
                       css1.style.height = '100%';
                       css1.style.background = 'rgba(0,0,0,0.5)';
                       css1.style.backdropFilter = 'blur(5px)';
                       css1.style.position = 'fixed';
                       css1.style.left = '0px';
                       css1.style.top = '0px';
                       css1.style.zIndex = '99999999';
                            ...
                            if(blzx.isMobile()){
                               ...
                               {$mheight}
                                                               ...
                                                               if(blzx.isMobile()){
                                div.style.width = '100%';
                                div.style.borderRadius = '10px 10px 0 0'; // 设置圆角
                                div.style.overflow = 'hidden';   // 隐藏超出部分
                  div.innerHTML='<i class=\"blzx-close2 layui-icon layui-icon-close\" onclick=\"tanchuang2()\"></i><iframe id=\"wolive-iframe\" src=\"'+web+'\" onload=\"pageOk()\"></iframe><div id=\"loading\"><img src=\"{$url}{$this->base_root}/assets/images/platform/loading-2.svg\"></div>'
                                ...
                                if(!blzx.isMobile()){
                                    ...
                                    div.style.borderRadius = '10px 10px 10px 10px'; // 设置圆角
                                    div.style.overflow = 'hidden';   // 隐藏超出部分
                                    // document.getElementById('blzxMinChatWindowDiv_yuan').style.right='6px';
                                    // document.getElementById('blzxMinChatWindowDiv_yuan').style.margin='15px';
                                    document.getElementById('blzxMinChatWindowDiv_yuan').style.right='15px';
                                    document.getElementById('blzxMinChatWindowDiv_yuan').style.bottom='20px';
                                    div.innerHTML='<iframe id=\"wolive-iframe\" src='+web+' onload=\"pageOk()\"></iframe><div id=\"loading\"><img src=\"{$url}{$this->base_root}/assets/images/platform/loading-2.svg\"></div>'
                                               document.body.appendChild(div);
                            }
                            ...
                            if(title == groupid){
                                s.style.display ='block';
                                // 让遮罩层显示
                                var mask = document.getElementById('css1');
                                if (mask) {
