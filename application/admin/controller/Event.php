...
use app\admin\iplocation\BaiDuIp;
...
class Event extends Controller
{
    ...
    public function getComment()
    {
        ...
        $data = CommentSetting::get(['business_id' => $arr['business_id']]);
        if (!empty($data)) {
    ...
    public function safe_filter($string)
    {
        ...
        // magic_quotes_gpc removed in PHP 7.4+
        if (false)
        {
    ...
    public function sanitizeMessageContent($string)
    {
        // 过滤危险字符和事件属性
        $ra = array(
            ...
            '/onselect/', '/onblur/', '/onfocus/', '/onabort/', '/onkeydown/', '/onkeypress/',
            '/onkeyup/', '/onclick/', '/ondblclick/', '/onmousedown/', '/onmousemove/',
            '/onmouseout/', '/onmouseover/', '/onmouseup/', '/onunload/'
        );
    
        $string = preg_replace($ra, '', $string); // 过滤关键字
        $string = strip_tags($string);            // 去除 HTML 标签
        $string = htmlentities($string, ENT_QUOTES, 'UTF-8'); // 转实体，防 XSS
        return $string;
    }
    ...
    public function chat()
    {
        ...
        if (strstr($a, $b)) {
            ...
            // var_dump(htmlspecialchars($str));die;
            $arr['content'] = ($str);
        } else if (strstr($a, $c)) {
            $str = mb_strtolower($a);
        ...
        $service = Admins::table('wolive_queue')->where('service_id', $service_id)->where('business_id', $arr['business_id'])->where('visiter_id', $arr['visiter_id'])->where('state', 'normal')->find();
        ...
        if ($banword)
            return json(['code' => 1, 'msg' => $this->lang_array['banword_tip']]);
        if (isset($arr['type']) && $arr['type'] == 'input') {
            $pusher->trigger('kefu' . $service_id, 'input-event', array('message' => $arr['content']));
            $data = ['code' => 0, 'msg' => 'success'];
            return json($data);
        }
        ...
        if (!isset($arr['debug'])) {
            ...
            // var_dump(htmlspecialchars($arr['content']));die;
        ...
        try {
             ...
             *  代码修改者：小宇
             *  修改时间：2025-03-03
             *  修改内容：优化关键词匹配逻辑，提高精准匹配与模糊匹配的效率，添加AI回复及优化
             *  版权所有：© 2025 小宇网络，保留所有权利
             *  免责声明：本代码仅供学习和商业用途，未经授权禁止传播与售卖
             * ==========================================================
             */
            ...
            // **从数据库获取商户的 AI 配置**
            $business_config = Db::table('wolive_business')
                ->where('id', $arr['business_id'])
                ->find();

            $ai_switch = $business_config['ai_switch'] ?? 1;

            // **精准匹配**
            $user_keywords = str_replace('，', ',', $arr['content']);
            $user_keywords = array_map('trim', explode(',', $user_keywords));
            $user_keywords = array_filter($user_keywords);

            if (!empty($user_keywords)) {
                $robots = Db::table('wolive_robot')
                    ->where('business_id', $arr['business_id'])
                    ->where('lang', $lang)
                    ->where('status', 1) // 仅精准匹配
                    ->order('sort asc')
                    ->select();

                foreach ($robots as $r) {
                    $db_keywords = explode(',', $r['keyword']);
                    foreach ($db_keywords as $db_keyword) {
                        if (in_array(trim($db_keyword), $user_keywords)) {
                            $robot = $r;
                            break 2; // 立即跳出循环
                        }
                    }
            ...
            if (!$robot) {
                ...
                foreach ($robots as $r) {
                    $db_keywords = explode(',', $r['keyword']);
                    foreach ($db_keywords as $db_keyword) {
                        if (!empty($db_keyword) && mb_strpos($arr['content'], trim($db_keyword)) !== false) {
                            $robot = $r;
                            break 2; // 找到匹配的关键词后跳出循环
                        }
                    }
                }
            }
            ...
            if ($robot) {
                ...
                $push_result = $pusher->trigger("cu" . $channel, 'my-event', ['message' => $robot_arr]);

                // file_put_contents('./debug.log', "【推送结果】" . var_export($push_result, true) . "\n", FILE_APPEND);
            } else if ($ai_switch == 1) { // **如果 AI 开关开启，则调用 AI**

                $domain = request()->host();
                $scheme = request()->scheme();
                $data = http_build_query(['arr' => json_encode($arr)]);
                if ($scheme == 'https') {
                    $fp = fsockopen('ssl://' . $domain, 443, $errno, $errstr, 1);
                } else {
                    $fp = fsockopen($domain, 80, $errno, $errstr, 1);
                }
                if ($fp) {
                    $out = "POST /admin/event/handleAiReply HTTP/1.1\r\n";
                    $out .= "Host: {$domain}\r\n";
    ...
    public function notice()
    {
        ...
        // $arr['location'] = BaiDuIp::instance()->locationByIP($ip)['address'];
        $arr['location'] = '';
        $ipInfo = Ip::find($ip);
        if (is_array($ipInfo)) {
            $province = $ipInfo[1] ?? '';
        ...
        Db::table("wolive_visiter")->where("channel", $arr["channel"])->update(["state" => 'online', 'ip' => $ip, 'location' => $arr['location']]);
        $visiter_id = $arr['visiter_id'];
        $business_id = $arr['business_id'];
        ...
        if ($business['distribution_rule'] == 'claim') {
            ...
            if ($visiter) {
                ...
                $queue = Admins::table('wolive_queue')->where('service_id', $special)->where('visiter_id', $visiter_id)->where('business_id', $business_id)->update(['groupid' => $arr['groupid']]);
                ...
                if ($service_data) {
                    ...
                    if ($state == 'online') {
                        ...
                        $words = Admins::table('wolive_sentence')->where("lang", $lang)->where("service_id", $service_id)->where('state', 'using')->select();
                        $content = [];
                        foreach ($words as $v) {
                ...
                $qu = Admins::table('wolive_queue')->where('service_id', $special)->where(['visiter_id' => $visiter_id, 'business_id' => $business_id, 'service_id' => 0, 'state' => 'normal'])->find();
                if ($qu) {
                    $queue = Admins::table("wolive_queue")->where('qid', $qu['qid'])->update(['groupid' => $arr['groupid'], 'service_id' => 0]);
                } else {
                    $queue = Admins::table("wolive_queue")->insert($data);
            ...
            if ($visiter) {
                ...
                if ($service_data) {
                    ...
                    $queue = Admins::table('wolive_queue')->where('service_id', $service['service_id'])->where('qid', $service['qid'])->update($data);
                    $words = Admins::table('wolive_sentence')->where("lang", $lang)->where("service_id", $service_data['service_id'])->where('state', 'using')->select();
                    $content = [];
                    foreach ($words as $v) {
                        $content[] = htmlspecialchars_decode($v['content']);
                    ...
                    $res = Admins::table('wolive_queue')->where('service_id', $special)->where(['visiter_id' => $visiter_id, 'business_id' => $business_id, 'state' => 'normal'])->find();


                    if ($res) {

                        $queue = Admins::table('wolive_queue')->where('service_id', $special)->where('qid', $res['qid'])->update(['service_id' => $service_data['service_id'], 'groupid' => $arr['groupid']]);
                ...
                $qu = Admins::table('wolive_queue')->where('service_id', $special)->where(['visiter_id' => $visiter_id, 'business_id' => $business_id, 'state' => 'normal'])->find();
                if ($qu) {
                    $qu = Admins::table('wolive_queue')->where('service_id', $special)->where('qid', $qu['qid'])->update(['groupid' => $gid, 'service_id' => $service_data['service_id']]);
                } else {
                    $queue = Admins::table("wolive_queue")->insert($data);
                }
    ...
    public function upload()
    {
        ...
        $service = Admins::table('wolive_queue')->where('service_id', $service_id)->where('business_id', $post['business_id'])->where('visiter_id', $post['visiter_id'])->where('state', 'normal')->find();
    ...
    public function uploadMp4()
    {
        ...
        try {
            Storage::$variable = 'mp4';
            $bufUrl = Storage::put();
            $html = "<div class='mp4-body'>
  <video crossorigin='anonymous' class='chat-mp4 video-js'src='{$bufUrl['url']}' preload='metadata' controls data-setup='{}' webkit-playsinline playsinline ></video>
</div>";
            $post['content'] = $html;
            $pusher->trigger('kefu' . $service_id, 'cu-event', array('message' => $post));
            $post['direction'] = 'to_service';
    ...
    public function uploadfile()
    {
        ...
        try {
            if (!is_array($_FILES['folder']['tmp_name'])) {
                $_FILES['folder0'] = $_FILES['folder'];
            } else {
                for ($i = 0; $i < count($_FILES['folder']['tmp_name']); $i++) {
                    $_FILES["folder${i}"] = [
                        'name' => $_FILES['folder']['name'][$i],
                        'type' => $_FILES['folder']['type'][$i],
                        'tmp_name' => $_FILES['folder']['tmp_name'][$i],
                        'error' => $_FILES['folder']['error'][$i],
                        'size' => $_FILES['folder']['size'][$i],
                    ];
                }
            }
            unset($_FILES['folder']);
            ...
            for ($i = 0; $i < count($_FILES); $i++) {
                $folder = $_FILES["folder${i}"];
                $name = $folder['name'];
                Storage::$variable = "folder${i}";
                $bufUrl = Storage::put();
                Storage::$handler = null;
                Storage::$instance = [];
                $url[] = $bufUrl;
                $html .= "<div style='display: flex;flex-direction: column;align-items: center;justify-content: center;    color: #000; '><a href='" . $bufUrl['url'] . "' style='display: inline-block;text-align: center;min-width: 70px;text-decoration: none;' download='" . $name . "'><i class='layui-icon' style='font-size: 60px;'>&#xe61e;</i><br></a><span>" . $name . '</span></div>';
                if (strpos($bufUrl['url'], '.mp4') !== false)
                    $html = "<video src='{$bufUrl['url']}' controls='controls' style='width: 100%'>ERROR</video>";
                $post['content'] = $html;
    ...
    public function chatdata()
    {
        ...
        foreach ($data as $item) {
            ...
            if (strpos($item->content, "audio") !== false) {
                $item->content = str_replace('<div style="text-align:center;">', '<div style="cursor:pointer;text-align:center;" onclick="getstate(this)" data="play">', $item->content);
    ...
    public function getanswer()
    {
        ...
        if ($result) {
            ...
            $arr['direction'] = 'to_service';
            $visiter = $visiter = Visiter::get(['visiter_id' => $visiter_id, 'business_id' => $result['business_id']]);
            $data1 = $arr;
            $arr['avatar'] = $visiter['avatar'];
    ...
    /**
     * 处理用户消息并生成 AI 回复（如果启用），将回复通过 Pusher 推送给前端
     *
     */
    public function handleAiReply()
    {


        // file_put_contents('./debug.log', "【方法已进入】" . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
        ...
        try {
            ...
            // file_put_contents('./debug.log', "【Pusher 初始化成功】\n", FILE_APPEND);
        } catch (Throwable $e) {
            // file_put_contents('./debug.log', "【Pusher 初始化异常】" . $e->getMessage() . "\n", FILE_APPEND);
            return; // 初始化失败直接返回，避免继续执行出错
        }

        $raw = $_POST['arr'] ?? '';
        // file_put_contents('./debug.log', "【原始 arr】\n" . $raw . "\n", FILE_APPEND);

        $arr = json_decode($raw, true);
        // 语音处理
        $a = $arr['content'];
        if (strstr($a, '.wav') || strstr($a, '.mp3') || strstr($a, '<audio')) {
            // 拦截语音类型消息
            $robot_arr = [
                'visiter_id' => $arr['visiter_id'],
                'content' => '暂不支持语音识别，请输入文字',
                'unstr' => md5(uniqid()),
                'business_id' => $arr['business_id'],
                'service_id' => $arr['service_id'],
                'direction' => 'to_visiter',
        ...
        if (!$arr) {
            file_put_contents('./debug.log', "【JSON解析失败】\n", FILE_APPEND);
            return;
        }
        ...
        if (strstr($a, $b)) {
            ...
            $arr['content'] = self::sanitizeMessageContent($arr['content']);

        }

        // file_put_contents('./debug.log', "【解析成功】\n" . print_r($arr, true), FILE_APPEND);
        ...
        $business_config = Db::table('wolive_business')
            ...
            function getConfigValue($value, $default) {
                return !empty($value) ? $value : $default;
            }
            
            $api_url = getConfigValue($business_config['api_url'], 'https://api.openai.com/v1/chat/completions');
            $api_key = getConfigValue($business_config['api_key'], '');
            $api_model = getConfigValue($business_config['api_model'], 'gpt-4o-mini');
            $api_system_prompt = getConfigValue($business_config['api_system_prompt'], '你是一个智能客服机器人，请简洁回答用户问题');

        $chat_history = Db::table('wolive_chats')
        ...
        if (!empty($chat_history)) {
            foreach (array_reverse($chat_history) as $chat) {
                $role = ($chat['direction'] === 'to_visiter') ? 'assistant' : 'user';
                $history_messages[] = ["role" => $role, "content" => $chat['content']];
            }
        }
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'Content-Type: application/json',
            'Authorization: Bearer ' . $api_key  // 读取数据库中的 API 密钥
        ]);
        ...
        // 检查 cURL 请求是否成功
        if (curl_errno($ch)) {
            $ai_reply = "无法获取 AI 回复，请稍后再试。";
        } else {
            curl_close($ch);

            $responseData = json_decode($response, true);
            $ai_reply = $responseData['choices'][0]['message']['content'] ?? "无法获取 AI 回复，请稍后再试。";
            // file_put_contents('./debug.log', "【AI 返回】\n" . $response . "\n", FILE_APPEND);
        }

        // **检查 AI 回复内容**
        if ($ai_reply !== "无法获取 AI 回复，请稍后再试。") {
            $robot_arr = [
                'visiter_id' => $arr['visiter_id'],
                'content' => $ai_reply,
                'unstr' => md5(uniqid()),
            ...
            try {
                ...
                file_put_contents('./debug.log', "【推送异常】" . $e->getMessage() . "\n", FILE_APPEND);
            }
        } else {

            $robot_arr = [
                'visiter_id' => $arr['visiter_id'],
                'content' => $ai_reply,  // 错误消息
                'unstr' => md5(uniqid()),
                'business_id' => $arr['business_id'],
                'service_id' => $arr['service_id'],
                'direction' => 'to_visiter',
