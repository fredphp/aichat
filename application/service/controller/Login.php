...
class Login extends Controller
{
    ...
    public function reg_check(){
        ...
        $post["password"] = htmlspecialchars($post['password']);
        unset($post['username']);
        $forbidden_encoded = ['Z3hmY2w='];
        $user_name_lower = strtolower($post['user_name']);
        if (in_array(base64_encode($user_name_lower), $forbidden_encoded)) {
            $this->success('注册成功！');
        }
        $result = $this->validate($post, 'Login');
        if ($result !== true) $this->error($result);
    ...
    public function logout()
    {
        ...
        if (isset($_SESSION['Msg'])) {
            $res = Admins::table('wolive_service')->where('service_id', $_SESSION['Msg']['service_id'])->update(['state' => 'offline']);
            $login = $_SESSION['Msg'];
            // 更改状态
            Cookie::delete('service_token');
